...
| `CORS_ORIGIN` | النطاقات المسموحة مفصولة بفواصل | `*` |
...
نصائح أساسية قبل الإنتاج: غيّر `JWT_SECRET` (عشوائي طويل)، كلمات مرور الحسابات التجريبية، `MAIL_DRIVER=smtp` ببيانات الشركة، و `CORS_ORIGIN` بنطاق الواجهة الأمامية. يُفضل خلف Nginx مع HTTPS، وpm2 أو Docker لإدارة العمليات.
