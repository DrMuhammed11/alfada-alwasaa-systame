...
export function generateStaticParams() {
  return Object.keys(EN_BLOG_POSTS).map((slug) => ({ slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = EN_BLOG_POSTS[slug];
  if (!post) return {};

  return {
    title: `${post.title} | Al-Fada Al-Wasaa Blog`,
    description: post.description,
    keywords: post.keywords,
    alternates: {
      canonical: `https://www.alfadaalwasaa.com/en/blog/${slug}`,
      languages: {
    ...
    openGraph: {
      title: post.title,
      description: post.description,
      url: `https://www.alfadaalwasaa.com/en/blog/${slug}`,
...
export default async function EnSingleBlogPostPage({ params }: Props) {
  const { slug } = await params;
  const post = EN_BLOG_POSTS[slug];
  if (!post) notFound();

  const otherPosts = Object.values(EN_BLOG_POSTS).filter((p) => p.slug !== slug);
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://www.alfadaalwasaa.com/en/blog/${slug}`,
    },
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: post.title,
        item: `https://www.alfadaalwasaa.com/en/blog/${slug}`,
      },
    ],
  };
        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-navy-darker via-navy to-navy py-14 text-white sm:py-20">
          ...
          <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <nav className="mb-6 flex items-center gap-2 text-xs font-semibold text-white/70" aria-label="Breadcrumb">
              ...
              <Link href="/en/blog" className="hover:text-gold transition">
                Blog
              </Link>
              <ChevronRight className="h-3 w-3 text-gold" />
              <span className="text-gold-light font-bold truncate max-w-[240px]">{post.category}</span>
            </nav>
        ...
        <article className="py-14 bg-white">
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-12 rounded-2xl bg-slate-50 border border-slate-200 p-6 sm:p-8">
              <h3 className="text-lg font-black text-navy mb-3">Bottom Line</h3>
            ...
            <div className="mt-12 rounded-3xl bg-navy p-8 text-white text-center sm:text-start sm:flex sm:items-center sm:justify-between gap-6">
              <div>
                <h3 className="text-xl font-black">Need engineering consultation for your project?</h3>
                <p className="text-xs text-white/75 mt-1">Connect directly with the engineers and specialists at Al-Fada Al-Wasaa.</p>
              </div>
              <Link
                href="/en/contact"
          ...
          <section className="py-14 bg-slate-50 border-t border-slate-200">
            <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-black text-navy">Other articles you may find interesting</h3>
                <Link href="/en/blog" className="inline-flex items-center gap-1 text-xs font-bold text-gold hover:text-gold-dark">
