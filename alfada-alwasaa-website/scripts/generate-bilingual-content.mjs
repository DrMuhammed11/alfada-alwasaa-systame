...
for (let i = 0; i < SITE_CONFIG.stats.length; i++) {
  const ar = SITE_CONFIG.stats[i];
  const en = EN_SITE_CONFIG.stats[i] || { label: "", sub: "" };
  md += `| **${ar.num}** | ${ar.label} | ${ar.sub} | ${en.label} | ${en.sub} |\n`;
}
...
for (let i = 0; i < EN_SITE_CONFIG.compliance.length; i++) {
  const item = EN_SITE_CONFIG.compliance[i];
  md += `| **${item.badge}** | ${item.title} | ${item.desc} | **${item.title}** (${item.subtitle}) | ${item.desc} |\n`;
}
...
for (const t of EN_SITE_CONFIG.testimonials) {
  md += `| **${t.name}**<br/>${t.role} | ${t.sector} | "${t.quote}" |\n`;
}
